...
st.caption('Uses scaled features, OLS significance filtering, multinomial logistic regression with softmax, and a gradient-descent regressor.')
...
if run:
    ...
    raw = load_data(ticker, str(start), str(end), interval)
    ...
    train_pred = add_predictions(train_df, scaler, clf, reg, selected_features)
    ...
    st.download_button('Download simulation results CSV', csv, file_name=f'{ticker}_softmax_simulation_results.csv', mime='text/csv')
else:
